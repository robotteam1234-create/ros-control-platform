...
export default function App() {
  ...
  return <main>
    ...
    <LineFollowPanel lease={lease?.lease_id ?? null} onError={setError} />
    <RecordPanel lease={lease?.lease_id ?? null} onError={setError} />
    <MappingPanel lease={lease?.lease_id ?? null} mapId={state?.map_id} onError={setError} />
    ...
    <StopBar role={user.role} lease={lease?.lease_id ?? null} selectedRobot={selectedRobot} />
    {cameraRobots.length > 0 && <CameraGrid robots={cameraRobots} initialQuality={cameraQuality} />}
    <Teleop robotId={selectedRobot} lease={lease?.lease_id ?? null} mode={state?.robots.find(r => r.robot_id === selectedRobot)?.mode ?? 'UNKNOWN'} stopLatched={state?.robots.find(r => r.robot_id === selectedRobot)?.stop_latched ?? null} />
  </main>
}
