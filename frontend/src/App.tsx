...
type Robot = { robot_id: string; name: string; role: 'MASTER' | 'SLAVE'; connection: string; mode: string; battery_percent: number | null; voltage_v?: number | null; linear_mps?: number | null; angular_rps?: number | null; pose?: { x: number; y: number; yaw: number } | null; pose_freshness?: string; stop_latched: boolean | null; capabilities?: string[]; sensors?: Array<{ name: string; state: string }>; trail?: Point[]; path?: Point[]; goal?: { x: number; y: number; yaw: number; frame_id: string } | null; tf_valid?: boolean; tf_reason_code?: string | null; received_at?: string | null }
...
export default function App() {
  ...
  // Camera tiles for every robot whose camera the backend actually subscribes
  // (sensors carry a camera entry only when camera.enabled in the bridge config).
  const cameraRobots = state?.robots.filter(robot => robot.sensors?.some(sensor => sensor.name === 'camera')) ?? []
  ...
  return <main>
    ...
    {activeTab === 'control' && (<><CameraGrid robots={cameraRobots} initialQuality={cameraQuality} />{cameraRobots.length === 0 && <div className="empty">카메라가 아직 연결되지 않았습니다. 로봇에서 camera publisher를 실행하면 자동으로 표시됩니다.</div>}<section><div className="section-title"><h2>로봇 상태</h2><span>{state?.server_time ? new Date(state.server_time).toLocaleTimeString('ko-KR') : '—'}</span></div><div className="pinky-cards">{state?.robots.map(r => <PinkyCard key={r.robot_id} robot={r} selected={selectedRobot === r.robot_id} onSelect={() => setSelectedRobot(r.robot_id)} lease={lease?.lease_id ?? null} onMode={m => setMode(r.robot_id, m).catch(e => setError(e.message))} />) ?? <div className="empty">상태를 기다리는 중입니다.</div>}</div></section><StopBar role={user.role} lease={lease?.lease_id ?? null} selectedRobot={selectedRobot} /><Teleop robotId={selectedRobot} lease={lease?.lease_id ?? null} mode={state?.robots.find(r => r.robot_id === selectedRobot)?.mode ?? 'UNKNOWN'} stopLatched={state?.robots.find(r => r.robot_id === selectedRobot)?.stop_latched ?? null} /></>)}
